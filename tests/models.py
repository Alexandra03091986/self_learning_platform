...
class Test(models.Model):
    ...
    lesson = models.OneToOneField(
        Lesson, on_delete=models.CASCADE, related_name="test", verbose_name="Урок", help_text="Выберите урок для теста"
    )
    title = models.CharField(max_length=200, verbose_name="Название теста", help_text="Введите название теста")
    description = models.TextField(
        verbose_name="Описание теста", help_text="Введите описание теста", blank=True, null=True
    )
    ...
    is_active = models.BooleanField(
        verbose_name="Активен", help_text="Отметьте, если тест доступен для прохождения", default=True
    )
...
class Question(models.Model):
    ...
    test = models.ForeignKey(
        Test, on_delete=models.CASCADE, related_name="questions", verbose_name="Тест", help_text="Выберите тест"
    )
    ...
    text = models.TextField(verbose_name="Текст вопроса", help_text="Введите текст вопроса")
    ...
    order = models.PositiveIntegerField(verbose_name="Порядок", default=0, help_text="Порядковый номер вопроса")
    explanation = models.TextField(
        verbose_name="Пояснение", help_text="Пояснение к правильному ответу", blank=True, null=True
    )
    created_at = models.DateTimeField(auto_now_add=True, verbose_name="Дата создания")
...
class AnswerOption(models.Model):
    ...
    text = models.CharField(max_length=500, verbose_name="Текст ответа", help_text="Введите текст ответа")
    is_correct = models.BooleanField(
        verbose_name="Правильный ответ", help_text="Отметьте, если это правильный ответ", default=False
    )
...
class TestAttempt(models.Model):
    ...
    user = models.ForeignKey(
        settings.AUTH_USER_MODEL, on_delete=models.CASCADE, related_name="test_attempts", verbose_name="Пользователь"
    )
    ...
    percentage = models.FloatField(null=True, blank=True, verbose_name="Процент выполнения")
    passed = models.BooleanField(null=True, blank=True, verbose_name="Тест сдан")
    ...
    class Meta:
        ...
        constraints = [
            models.UniqueConstraint(
                fields=["test", "user"], condition=models.Q(status="in_progress"), name="unique_active_attempt"
            )
        ]
...
class UserAnswer(models.Model):
    ...
    selected_options = models.ManyToManyField(
        AnswerOption, blank=True, verbose_name="Выбранные варианты", related_name="user_answers"
    )
...
class TestResult(models.Model):
    ...
    user = models.ForeignKey(
        settings.AUTH_USER_MODEL, on_delete=models.CASCADE, related_name="test_results", verbose_name="Студент"
    )
    test = models.ForeignKey(Test, on_delete=models.CASCADE, related_name="results", verbose_name="Тест")
    attempt = models.OneToOneField(
        TestAttempt, on_delete=models.CASCADE, related_name="result", verbose_name="Попытка", null=True, blank=True
    )
    ...
    completed_at = models.DateTimeField(verbose_name="Дата прохождения", auto_now_add=True)
