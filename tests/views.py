...
from .models import AnswerOption, Test, TestAttempt, TestResult, UserAnswer
from .paginations import TestPagination, TestResultPagination
from .permissions import CanCreateTest, CanTakeTest, IsAttemptOwner, IsTestOwnerOrAdmin
from .serializers import (
    TestAttemptSerializer,
    TestDetailStudentSerializer,
    TestListSerializer,
...
class TestViewSet(viewsets.ModelViewSet):
    ...
    def perform_create(self, serializer):
        ...
        if self.request.user.role == "teacher":
            if lesson.owner != self.request.user:
                raise PermissionDenied("Вы можете создавать тесты только для своих уроков")
    ...
    def start_attempt(self, request, pk=None):
        ...
        attempts_count = TestAttempt.objects.filter(test=test, user=request.user).exclude(status="in_progress").count()
        ...
        active_attempt = TestAttempt.objects.filter(test=test, user=request.user, status="in_progress").first()

        if active_attempt:
            serializer = TestAttemptSerializer(active_attempt)
            return Response({"message": "У вас уже есть активная попытка", "attempt": serializer.data})
    ...
    def submit(self, request, pk=None):
        ...
        attempt = TestAttempt.objects.filter(test=test, user=request.user, status="in_progress").first()
        ...
        if UserAnswer.objects.filter(attempt=attempt).exists():
            raise ValidationError(
                f"Вы уже отправили ответы на попытку #{attempt.id}. " f"Для новой попытки вызовите start_attempt"
            )
    ...
    def _finish_attempt(self, test, attempt, total_points, max_points):
        ...
        completed_count = TestAttempt.objects.filter(test=test, user=attempt.user, status="completed").count()
